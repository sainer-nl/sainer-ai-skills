...
- **Transfers** — can it transfer calls? For each destination, help write its **description** (when to route there). Capture cross-cutting rules — urgency overrides, tiebreakers, "never ask X just to route", how to frame a stand-in for a closed destination — as **routing guidance**. If some destinations' staff speak only a subset of the operator's languages, recommend setting that destination's spoken-languages for an automatic heads-up.
